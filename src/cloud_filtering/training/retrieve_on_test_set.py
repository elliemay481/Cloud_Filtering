...
def load_model(tag):
    ...
    model = CloudSignalModel_MultiOutput(
        n_inputs=len(input_variables), n_outputs=n_quantile_outputs, **config.MODEL
    )
...
def create_dataset_of_retrievals(
    filename,
    input_variables,
    output_variables,
    tag,
    ...
    ds_out["CloudSat_Datetime"] = xr.DataArray(
        ds["CloudSat_Datetime"][mask], dims=("number",)
    )
    ...
    for var in output_variables:
        ...
        ds_out[f"{var}_quantiles"] = xr.DataArray(
            y_quantiles[var], dims=("number", "quantile")
        )
...
def run_batch(x, start, end, output_variables, y_quantiles, y_mean, mrnn):
    ...
    for var in output_variables:
        y_quantiles[var][start:end, :] = (
            y_pred_quantiles[var].detach().numpy().astype(np.float32)
        )
...
if __name__ == "__main__":
    ...
    surface_masks_test = retrieval_preprocessing.surface_mask(
        filename=config.AWS_TEST_SET
    )
    ...
    for tag in config.VARIANTS.keys():
        ...
        x_test, y_test, _, surface_mask = retrieval_preprocessing.prepare_data(
            tag,
            config.AWS_TEST_SET,
            input_variables,
            output_variables,
        ...
        for i, ch in enumerate(input_variables[:-1]):  # all except mirror angle
            x_test_np[:, i] += np.random.normal(
                0, config.AWS_CHANNEL_NOISE[ch], len(x_test_np[:, i])
            )
        ...
        y_quantiles = {
            var: np.empty((N_samples, config.N_QUANTS), dtype=np.float32)
            for var in output_variables
        }
        y_mean = {
            var: np.empty(N_samples, dtype=np.float32) for var in output_variables
        }
        ...
        for start in range(0, int(N_samples), int(batch_size)):
            ...
            y_quantiles, y_mean = run_batch(
                x_test_np, start, end, output_variables, y_quantiles, y_mean, mrnn
            )

        ds_retrievals = create_dataset_of_retrievals(
            config.AWS_TEST_SET,
            input_variables,
            output_variables,
            tag,
        ...
        out_path = (
            config.DATA_DIR / f"cloud_signal_test_set_retrievals_{tag}_{today}.nc"
        )
